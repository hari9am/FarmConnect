...
// Simple selectable map using Leaflet
...
export default function AddUpcomingCrop() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Note: upcomingCrops schema does not currently accept location.
      // If you want to persist location, we can extend the schema and backend.
      await apiRequest("POST", "/api/upcoming", payload);
      // Set flags so dashboards can show a confirmation banner/notice
      try {
        localStorage.setItem("fc-recent-upcoming", "1");
        localStorage.setItem("fc-upcoming-notice", "1");
      } catch {}
      toast({ title: t("saved") || "Saved", description: t("upcoming_crop_added") || "Upcoming crop added." });
      navigate("/farmer/dashboard");
    } catch (err: any) {
      toast({ title: t("failed") || "Failed", description: err?.message || t("unable_add_upcoming") || "Unable to add upcoming crop.", variant: "destructive" });
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="flex flex-col min-h-screen">
        <header className="p-4 border-b border-border bg-card">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-lg font-semibold">{t("add_upcoming_crop") || "Add Upcoming Crop"}</h1>
          </div>
        </header>
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 mt-1">
                  {/* Duration value (number entered manually) */}
                  <Input
                    type="number"
                    min={0}
                    placeholder="0"
                    value={(() => {
                      ...
                      if (months !== "") return String(months);
                    ...
                    onChange={(e) => {
                      ...
                      // Keep whichever unit is currently selected based on which state is non-empty
                      if (years !== "") { setYears(num as any); setMonths(""); setDays(""); }
                        ...
                        // Default to months if no unit chosen yet
                        setMonths(num as any); setYears(""); setDays("");
                      }
                    }}
                  />

                  {/* Unit dropdown (only Years/Months/Days) */}
                  <select
                    className="h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                    value={years !== "" ? "y" : months !== "" ? "m" : days !== "" ? "d" : ""}
                    onChange={(e) => {
                      const unit = e.target.value; // '', 'y', 'm', 'd'
              ...
              <Button type="submit" className="w-full" disabled={submitting}>
                {submitting ? "Saving..." : (t("save_upcoming_crop") || "Save Upcoming Crop")}
              </Button>
            </CardContent>
          </Card>
        </form>
